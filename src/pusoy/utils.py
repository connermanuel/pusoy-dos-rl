...
class RoundType(Enum):
    ...
    def to_tensor(self, dtype=torch.bool):
        ...
        value = min(
            self.value, 4
        )  # Maps hands to the 5th index, where it should belong.
        tensor[value] = 1
        return tensor
...
class PrioritizedItem:
    """
    A class that groups a priory and a item, so that they can be arranged and ordered
...
def count_cards_per_value(cards: torch.Tensor) -> torch.Tensor:
    """For each value, determine how many cards you have."""
