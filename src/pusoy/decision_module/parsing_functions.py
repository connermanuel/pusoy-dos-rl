...
ParsingFunction = Callable[
    [torch.Tensor, torch.Tensor, torch.Tensor, Hands, bool, bool, SelectionFunction],
    Action | None,
]
...
def find_best_k_of_a_number(
    ...
    best_move_idx = selection_function(torch.Tensor(scores), 1).item()
...
def find_best_straight(
    ...
    best_idxs[4] = last_max_idxs[best_option_value + 4] + ((best_option_value + 4) * 4)
...
def find_best_full_house(
    ...
    best_triple = find_best_triple(
        card_probs, card_list, prev_play, hand_type, is_first_move, selection_function
    )
    ...
    if best_triple.cards[0]:
        is_first_move = False
    # Pair has not torch.any(prev_play) enabled, because you can do any pair
