...
def ppo_loss(curr_model: torch.nn.Module, prev_model: torch.nn.Module, inputs: List[torch.tensor], 
            ...
            gamma: float=0.99, lambd: float=0.9, c_entropy: float=0.001, device: str='cuda'):
    """
    ...
    - inputs: list[torch.tensor] -- (batch_size, seq_len, input_dim) the input to the model representing the game state
    ...
    inputs = [torch.stack(t) for t in inputs]
    input = pad_sequence(inputs, padding_value=0).to(device) # Now forms a sequence of inputs
    input = pack_padded_sequence(input, lengths=lengths, enforce_sorted=False)
    output, values, _ = curr_model(input, packed=True)
    ...
    adv, critic_loss = gae(values, gamma, lambd, rewards, device, cum_lengths)
    surr1 = ratios * adv
    surr2 = torch.clamp(ratios, 1-eps_clip, 1+eps_clip) * adv

    return -torch.min(surr1, surr2).mean() + critic_loss - c_entropy * entropy(curr_log_probs, SOFTMAX_SIZES)
...
def batch_generate_mask(actions, device):
    ...
    card_tensors, round_tensors, hand_tensors = tuple(zip(*[(action.cards, action.type.to_tensor(), action.hand.to_tensor()) 
