...
from pusoy.utils import RoundType, Hands, idx_to_card, count_cards_per_value, print_cards
import torch
...
class PlayCards(Action):
    ...
    def ValidCards(self, cards, player, game):
        ...
        if torch.any(invalid_cards):
            ...
            raise ValueError(f"You don't have {idx_to_card(invalid_cards_idxs[0].item())}!")
    ...
    def ValidValue(self, cards, player, game):
        if game.prev_play is not None:
            ...
            if curr_max < prev_max:
                raise ValueError(f"The highest card in the previous move was {idx_to_card(prev_max)}, and you played {idx_to_card(curr_max)}!")
    ...
    def ValidHands(self, cards, player, game):
        ...
        elif most_frequent_count == 3 and torch.nonzero(count_cards_per_value(cards)).flatten().shape == (2,):
        ...
        if game.hand_type:
            if hand < game.hand_type:
                raise ValueError(f'The last move was a {game.hand_type} which is higher than your {hand}!')
    ...
    def ValidHandValue(self, cards, player, game, hand):
        if hand == Hands.STRAIGHT or hand == Hands.FLUSH or hand == Hands.STRAIGHT_FLUSH:
            self.ValidValue(cards, player, game)
            ...
            if value < prev_value:
                raise ValueError(f'The last move was a {game.round_type} of {prev_value}, which is higher than your {value}!')
